...
/// Overwrites the current hash value with the hash of itself ++ mixin (if provided)
pub fn rehash(prior: *Hash, maybe_mixin: ?Hash) void {
    // TODO: verify that the compiler optimizes away branching when mixin is hardcoded
    if (maybe_mixin) |mixin| {
        // TODO: is concatenation correct? is there a faster way?
        std.crypto.hash.sha2.Sha256.hash(prior ++ &mixin, prior, .{});
    } else {
        std.crypto.hash.sha2.Sha256.hash(prior, prior, .{});
    }
}
