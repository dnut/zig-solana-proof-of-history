...
const SendError = @import("collections.zig").SendError;
...
pub const PohRecorder = struct {
    ...
    sender: *AtomicQueue(WorkingBankEntry),
    ...
    pub fn init(allocator: std.mem.Allocator, start_hash: Hash, hashes_per_tick: u64, bank: *Bank, sender: *AtomicQueue(WorkingBankEntry)) PohRecorder {
        return PohRecorder{
    ...
    pub fn record(self: *Self, _: Slot, hash: Hash, transactions: []const Transaction) PohRecorderError!?usize {
    ...
    pub fn tick(self: *Self) !void {
        if (self.poh.tick()) |poh_entry| {
            ...
            if (self
                .leader_first_tick_height_including_grace_ticks) |_|
            {} else {
                return;
            }
